// Test script for the deployed simple-events function specifically for party events
import fetch from 'node-fetch';

async function testSimpleEventsParty() {
  try {
    ...
    if (data.events && data.events.length > 0) {
      ...
      data.events.forEach((event, index) => {
        ...
        // Check if the event title or description contains party-related keywords
        const title = event.title.toLowerCase();
        const description = (event.description || '').toLowerCase();
        const containsPartyKeyword = partyKeywords.some(keyword => 
          title.includes(keyword) || description.includes(keyword)
        );
        
        console.log(`Contains party keyword: ${containsPartyKeyword}`);
      });
    } else {
