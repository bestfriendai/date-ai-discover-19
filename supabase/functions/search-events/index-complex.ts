// Updated search-events function with improved error handling and performance
...
try {
  ...
  console.error('Error importing cors:', e);
}

// Import types with error handling
type Event = any;
type SearchParams = any;
type SourceStats = any;
type SearchEventsResponse = any;
try {
  const typesModule = await import("./types.ts");
  Event = typesModule.Event;
  SearchParams = typesModule.SearchParams;
  SourceStats = typesModule.SourceStats;
  SearchEventsResponse = typesModule.SearchEventsResponse;
} catch (e) {
  console.error('Error importing types:', e);
}
// Import the fixed PredictHQ integration
try {
  var { fetchPredictHQEvents } = await import("./predicthq-fixed.ts");
} catch (e) {
  console.error('Error importing predicthq-fixed:', e);
  // Provide fallback implementation
  var fetchPredictHQEvents = async () => ({ events: [], error: 'Module not available' });
}

// Import party detection utilities
try {
  var { detectPartyEvent, detectPartySubcategory } = await import("./partyUtils.ts");
} catch (e) {
  console.error('Error importing partyUtils:', e);
  // Provide fallback implementations
  var detectPartyEvent = () => false;
  var detectPartySubcategory = () => 'general';
}
// Import helper functions
try {
  var { calculateDistance, extractNumericPrice, parseEventDate } = await import("./helpers.ts");
} catch (e) {
  console.error('Error importing helpers:', e);
  // Provide fallback implementations
  var calculateDistance = (lat1: number, lon1: number, lat2: number, lon2: number) => 0;
  var extractNumericPrice = (priceStr: string | undefined) => 0;
  var parseEventDate = (date: string, time: string) => new Date();
}

// Custom error class for better error handling
class SearchEventsError extends Error {
  status: number;
  source: string;
  details?: any;

  constructor(message: string, status = 500, source = 'unknown', details?: any) {
    super(message);
    this.name = 'SearchEventsError';
    this.status = status;
    this.source = source;
    ...
    Object.setPrototypeOf(this, SearchEventsError.prototype);
  }
}
...
serve(async (req: Request) => {
  ...
  const responseHeaders = {
    ...
    'X-Function-Version': '1.2.0' // Track function version
  };
  ...
  try {
    ...
    // Validate and load API keys with fallbacks for testing
    ...
    if (!TICKETMASTER_KEY) {
      ...
      // Instead of throwing, we'll continue but log the error
      // This allows the function to still return results from other sources
      ticketmasterError = 'API key is not set';
    }

    if (!PREDICTHQ_API_KEY) {
      console.warn('[WARNING] PREDICTHQ_API_KEY is not set - PredictHQ events will not be available');
    }

    if (!SERPAPI_KEY) {
      console.warn('[WARNING] SERPAPI_KEY is not set - Google Events will not be available');
    }
      ...
      if (contentType.includes('application/json')) {
        ...
        if (body && body.trim()) {
          try {
            ...
            console.error('[SEARCH-EVENTS] Failed to parse JSON body:', e);
            throw new SearchEventsError('Invalid JSON in request body', 400, 'request');
          }
        }
      }
      console.log('[SEARCH-EVENTS] Request parameters:', JSON.stringify(params, null, 2));
    } catch (e) {
      console.error('[SEARCH-EVENTS] Error parsing request:', e);
      throw new SearchEventsError('Failed to parse request', 400, 'request');
    }
    ...
    try {
      // Skip Ticketmaster API call if the API key is missing
      if (!TICKETMASTER_KEY) {
        console.log('[SEARCH-EVENTS] Skipping Ticketmaster API call due to missing API key');
        ticketmasterCount = 0;
        ticketmasterError = 'API key is not set';
      } else {
    ...
    if (error instanceof SearchEventsError) {
      errorMessage = error.message;
      errorStack = error.stack || '';
      errorType = error.name;
      errorStatus = error.status;
      errorSource = error.source;
      console.error(`[SEARCH-EVENTS] ${errorType} from ${errorSource}: ${errorMessage}`);
