 ...
 */
export function transformSeatGeekEvent(input: SeatGeekEvent): Event {
 ...
 */
export function extractSeatGeekParams(params: SearchParams, apiKey: string): SeatGeekParams {
 ...
 */
export async function searchSeatGeekEvents(params: SearchParams): Promise<Event[]> {
  console.log('[SEATGEEK] Starting SeatGeek event search');
  
  try {
    // Get API key from environment variables
    const seatgeekKey = apiKeyManager.getActiveKey('seatgeek');
    ...
    // Track API key usage
    apiKeyManager.trackKeyUsage('seatgeek', response.status !== 200);
