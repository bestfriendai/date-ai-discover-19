...
import { Search, X, Filter, Music, Calendar, MapPin, SlidersHorizontal, Sparkles, Clock } from 'lucide-react';
...
const PartySidebar: React.FC<PartySidebarProps> = ({
  ...
  const sortEvents = (events: Event[]) => {
    if (sortBy === 'date') {
      ...
      // Sort by popularity (using a heuristic - events with images and prices might be more popular)
      return [...events].sort((a, b) => {
        ...
        {isLoading ? (
          ...
          >
            <div className="relative mb-6">
              <Music className="h-16 w-16 text-purple-500/50" />
              <motion.div
              ...
              >
                <Music className="h-16 w-16 text-purple-500/20" />
              </motion.div>
            </div>
