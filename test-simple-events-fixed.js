// Test script for the simple-events function
import fetch from 'node-fetch';

async function testSimpleEvents() {
  try {
    ...
    // Count party subcategories
    const partySubcategoryCounts = {};
    data.events
      .filter(event => event.category === 'party' && event.partySubcategory)
      .forEach(event => {
        partySubcategoryCounts[event.partySubcategory] = (partySubcategoryCounts[event.partySubcategory] || 0) + 1;
      });

    if (Object.keys(partySubcategoryCounts).length > 0) {
