...
async function testRapidAPIConnection() {
  ...
  // Hardcode the endpoint since it's missing from the .env file
  const rapidApiEndpoint = 'https://real-time-events-search.p.rapidapi.com/search-events';
  ...
  try {
    ...
    queryParams.append('date', 'week'); // Using 'week' as in the original test
    ...
    if (data.data && Array.isArray(data.data)) {
      console.log(`Events received: ${data.data.length}`);
      ...
      if (data.events.length > 0) {
        ...
        data.events.slice(0, 2).forEach((event, index) => {
          console.log(`\n--- Event ${index + 1}: ${event.title || 'No Title'} ---`);
          console.log(`Category: ${event.category || 'No Category'}`);
          console.log(`Start Date: ${event.start_date || 'No Start Date'}`);
          console.log(`Venue: ${event.venue?.name || 'No Venue'}`);
          console.log(`Location: ${[event.venue?.city, event.venue?.state, event.venue?.country].filter(Boolean).join(', ') || 'No Location'}`);
