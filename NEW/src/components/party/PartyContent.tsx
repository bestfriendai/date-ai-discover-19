...
export const PartyContent: React.FC<PartyContentProps> = ({
  leftSidebarOpen,
  rightSidebarOpen,
  selectedEvent,
  showSearch,
  ...
  // Group events by subcategory for better organization
  const groupedEvents = React.useMemo(() => {
    const groups: Record<string, Event[]> = {};
    
    events.forEach(event => {
      const subcategory = event.partySubcategory || 'general';
      if (!groups[subcategory]) {
        groups[subcategory] = [];
      }
      groups[subcategory].push(event);
    });
      ...
      <div className={`fixed inset-y-0 left-0 z-20 w-80 bg-gray-900 transform transition-transform duration-300 ease-in-out ${leftSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full flex flex-col">
          <div className="p-4 border-b border-gray-800 flex justify-between items-center">
            <h2 className="text-xl font-bold">Party Events</h2>
            <button 
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {isEventsLoading ? (
              ...
              <div className="space-y-6">
                {Object.entries(groupedEvents).map(([subcategory, subcategoryEvents]) => (
                  <div key={subcategory} className="space-y-4">
                    <h3 className="text-lg font-semibold capitalize">
                      {subcategory.replace('-', ' ')} Parties
                      <span className="ml-2 text-sm text-gray-400">({subcategoryEvents.length})</span>
                    </h3>
                    
                    {subcategoryEvents.map(event => (
                      <div 
                        key={event.id}
                        className={`p-4 rounded-lg cursor-pointer transition-colors ${selectedEvent?.id === event.id ? 'bg-purple-900' : 'bg-gray-800 hover:bg-gray-700'}`}
              ...
              <div className="text-center py-8">
                <p className="text-gray-400">No party events found</p>
                <p className="text-sm text-gray-500 mt-2">Try searching in a different area</p>
              </div>
            )}
          </div>
        </div>
            ...
            {selectedEvent ? (
              <div>
                ...
                {selectedEvent.partySubcategory && (
                  <div className="mb-3">
                    <span className="inline-block px-3 py-1 bg-purple-800 rounded-full text-sm capitalize">
                ...
                <div className="flex space-x-2">
                  {selectedEvent.url && (
                    <a 
                      ...
                      className="flex-1 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg text-center transition-colors"
                    >
                      Party Website
                    </a>
                  )}
                  ...
                  <button 
                    ...
                    className="flex-1 py-2 bg-pink-600 hover:bg-pink-700 rounded-lg transition-colors"
                  >
                    Add to Plan
                  </button>
                </div>
              ...
              <div className="text-center py-8">
                <p className="text-gray-400">Select a party to view details</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex-1 relative">
        ...
        {mapHasMoved && (
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10">
            <button
              ...
              className="py-2 px-4 bg-purple-600 hover:bg-purple-700 rounded-lg transition-colors shadow-lg"
            >
              Find Parties Here
            </button>
          </div>
        ...
        <div className="absolute bottom-4 left-4 z-10 bg-gray-900 py-1 px-3 rounded-full text-sm">
          {isEventsLoading ? (
            ...
            <span>{events.length} parties found</span>
          )}
        </div>
      </div>
    </>
